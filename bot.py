...
# стратовая функция в ней создаем меню перехода и записываем пользователя в БД
@bot.message_handler(commands=['start', 'help', 'menu'])
def start(message=None, chat_id=None):
    user = get_user(message.chat.id if message else chat_id)
    menu = [['Избранное', 'Калькулятор артефактов', 'Рейтинг'], "Резонаторы"]
...
#декоратор для обработки inline кнопок(всех)
...
def callback_inline(call: types.CallbackQuery):
    ...
    if 'favorite' in call.data:
        ...
        if command == 'add':
            ...
            bot.send_message(user_id, f'🔥 Ваш резонатор {int(guide_id)} добавлено в избранное 🔥')
        ...
        if command == 'remove':
            ...
            bot.delete_message(user_id, call.message.message_id)
            bot.send_message(user_id, f'💔 Ваш резонатор {guide_id} удалено из избранного 💔')
            start(chat_id=user.chat_id)
...
# отправить рейтинг всех резонаторов
def send_rating(call):
    ...
    keyboard = Keyboa(items=menu, copy_text_to_callback=True)
    bot.send_message(call.message.chat.id, "🌟 Рейтинг резонаторов 🌟", reply_markup=keyboard())
...
def constellation_menu(message):
    ...
    bot.send_message(message.chat.id,
                     "Выбери резонатора в меню снизу 👇. Или отправь мне часть его имени, я попробую его найти 🔭",
                     reply_markup=markup)
...
def send_favorite(call):
    ...
    if len(favorites) == 0:
        bot.send_message(call.chat.id, "💔 Ваш список избранных резонаторов пуст 💔")
        return
    ...
    keyboard = Keyboa(items=menu, copy_text_to_callback=True)
    bot.send_message(call.chat.id, "\U0001F497 Ваш список избранных резонаторов \U0001F497", reply_markup=keyboard())
...
{talent_string}
    ...
    bot.send_message(chat_id, "🌟 rating menu 🌟 \n⭐Cохраните персонажа в избранное, чтобы вернуться к нему позже⭐", reply_markup=fav_guide_kb)
...
def get_guide(message):
    ...
    if len(guide) > 1:
        ...
        bot.send_message(message.chat.id, f"Возможно вы иммели введу этих резонаторов? 👇", reply_markup=keyboard)
        return
    if guide is None:
        bot.send_message(message.chat.id, "Я вас не понял \U0001F494")
        return
    if len(guide) == 0:
        bot.send_message(message.chat.id, "Я не могу найти это резонатора \U0001F494")
        return
    guide = guide[0]
